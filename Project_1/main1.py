...
from helpers import free_seo_audit,ai_analysis, display_wrapped_json
        ...
        if st.button("Start Analysis"):
            ...
            for i, url in enumerate(urls):
                ...
                with st.expander(f"SEO Audit for {url}", expanded=False):
                    report = full_seo_audit(url)  # Only using the comprehensive audit
                    
                    ai_tab, seo_tab = st.tabs(["AI Analysis", "SEO Report"])
                    
                    with ai_tab:
                        st.subheader("AI Analysis")
                        st.write(ai_analysis(report))
